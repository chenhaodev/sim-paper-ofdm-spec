...
%for thresh = 20:5:50 %ED
    ...
    for kk=1:iter % Number of Monte Carlo Simulations
        ...
        %{
        % test with signal exist
        [hat_m_M1, Sx_M1, feature_mask] = disp_sparse_cyclic_spec_t11(x);
        out1 = feature_mask.*abs(Sx_M1);
        csd_sig(kk) = (1/length(N)).*norm(out1);
        % test with signal non-exist (noise)
        [hat_m_M2, Sx_M2, feature_mask] = disp_sparse_cyclic_spec_t11(n);
        out2 = feature_mask.*abs(Sx_M2);
        csd_nos(kk) = (1/length(N)).*norm(out2);
        %}
        
        % test with signal exist
        [out1] = disp_sparse_cyclic_spec_t11(x);
        cs_csd_sig(kk) = (1/length(N)).*norm(out1);
        % test with signal non-exist (noise)
        [out2] = disp_sparse_cyclic_spec_t11(n);
...
%save roc_ed_0db_t11.mat Pfa_ed Pd_ed
%save roc_csd_0db_t11.mat Pfa_csd Pd_csd
save roc_cs_csd_0db_t11_C.mat Pfa_cs_csd Pd_cs_csd
