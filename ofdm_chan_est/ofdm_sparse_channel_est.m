...
% ofdm parameters
    ...
    ofdm.M  =  16;                       % Modulation order (M=4)
...
% building dictionary (Gamma, please check different papers to learn how to build the dictionary)
    ...
    chan.Gamma = exp(-sqrt(-1)*2*pi.*repmat(((1:ofdm.N).'),1,chan.Nt)./ofdm.TS.*repmat(chan.tau_p,ofdm.N,1));  %1D-dft dictionary
% or fft matrix for LSE estimation (please check the following link for LSE channel estimation :
% http://www.mathworks.com/matlabcentral/fileexchange/13127-ofdm-lse-channel-estimation )
%    F = exp(2*pi*sqrt(-1)/ofdm.N .* meshgrid(0:ofdm.N-1,0:ofdm.N-1).* repmat((0:ofdm.N-1)',[1,ofdm.N]));
